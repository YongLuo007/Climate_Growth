...
for(indispecies in studySpecies){
  ...
  indiANOVA <- indiANOVA[,.(Variable = rn, SumOfSq = `Sum Sq`, FValue = `F value`)]
  ...
  indiCoeff <- indiCoeff[,.(Variable = rn, Value = Estimate, SE = `Std. Error`)]
  indiCoeff[, Variable:=unlist(lapply(Variable,
                               function(s) 
                                 paste(sort(unlist(strsplit(s, ":", fixed = TRUE))), collapse = ":")))]
  names(indiCoeff)[2:3] <- paste(indispecies, "_", names(indiCoeff)[2:3], sep = "")
  allCoeffs <- dplyr::left_join(allCoeffs, indiCoeff, by = "Variable")
  allCoeffs <- dplyr::left_join(allCoeffs, indiANOVA, by = "Variable")
}
workPath <- "~/Github/Climate_Growth"
write.csv(allCoeffs, file.path(workPath, "Results", "Table1.csv"),
          row.names = FALSE)
