...
# output <- data.table(Species = character(), sizeIndex = character(),
#                      disweight = numeric(), IntraHAIC = numeric(), InterHAIC = numeric())
# for(sizeIndex in c("DBH", "Biomass")){
...
#                      data = speciesData,
#                      control = lmeControl(opt="optim", maxIter=50000, msMaxIter = 50000))
#       aictable <- data.table(Species = indispecies, sizeIndex = sizeIndex, 
...
#   }
#   cat("sizeIndex: ", sizeIndex, "is done. \n")
# }
# 
# output <- read.csv(file.path("~/GitHub/Climate_Growth/Results/disweightselection.csv"), header = T,
#                     stringsAsFactors = F) %>% data.table
# a <- melt(output, id.vars = c("Species", "sizeIndex", "disweight"), 
...
#                                     "Other species")))]
# 
# a[,minvalue:=min(Value), by = c("Species", "sizeIndex", "variable")]
# 
# minvaluepoints <- a[Value == minvalue,]
# 
# figure <- ggplot(data=a, aes(x = disweight, y = Value))+
#   facet_grid(Species~variable, scales = "free_y")+
#   geom_line(aes(group = sizeIndex, col = sizeIndex), size = 1)+
#   geom_point(data = minvaluepoints, aes(x = disweight, y = Value, col = sizeIndex),
...
output <- read.csv(file.path("~/GitHub/Climate_Growth/Results/bestWeightsbothsizeanddistanceRelativeSize.csv"),
...
for(indispecies in c("All species", "Jack pine", "Trembling aspen", "Black spruce",
  ...
  bestIntraHdata <- HeghyiCICalculation(data = processCIdata,
                                ...
                                sizeWeight = bestIntraSizeWei)
  bestInterHdata <- HeghyiCICalculation(data = processCIdata,
                                        maxRadius = 12.62,
                                        sizeIndex = "Biomass",
                                        distanceWeight = bestInterDisWei, 
                                        sizeWeight = bestInterSizeWei)
  
  bestIntraHdata <- bestIntraHdata[, .(uniTreeID=paste(PlotNumber, "_", TreeNumber, sep = ""),
                       IniYear = Year,
                       IntraH, InterH)] %>%
    unique(., by = c("uniTreeID", "IniYear"))
  bestInterHdata <- bestInterHdata[, .(uniTreeID=paste(PlotNumber, "_", TreeNumber, sep = ""),
                                       IniYear = Year,
                                       IntraH, InterH)] %>%
    unique(., by = c("uniTreeID", "IniYear"))
  bestIntraHdata[,InterH:=NULL]
  bestInterHdata[,IntraH:=NULL]
...
write.csv(allnewdata, file.path("~/GitHub/Climate_Growth/data/newAllData.csv"), row.names = F)
