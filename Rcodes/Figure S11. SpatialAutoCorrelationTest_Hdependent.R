...
for(indispecies in studySpecies){
  ...
  for(indiplot in selectedplots){
    ...
    if(indispecies == studySpecies[1] & indiplot == selectedplots[1]){
      ...
      allInteractions <- rbind(allInteractions, plotcoeffs)
    }
  }
}
...
legendPlot <- ggplot(data = data.frame(x = c(min(allInteractions$Interaction), 
                                             max(allInteractions$Interaction)),
                                       y = c(0, 5)), aes(x = x, y = y)) +
  ...
  geom_point(data = data.frame(y = 1.3, x = seq(min(allInteractions$Interaction),
                                                max(allInteractions$Interaction),
                                                length = 7),
                               texts = sprintf("%0.3f", 
                                               round(seq(min(allInteractions$Interaction),
                                                         max(allInteractions$Interaction),
                                                         length = 7), 3))),
             aes(x = x, y = y), pch = 17, size = 2)+
  geom_text(data = data.frame(y = 1, x = seq(min(allInteractions$Interaction),
                                             max(allInteractions$Interaction),
                                             length = 7),
                              texts = sprintf("%0.3f", 
                                              round(seq(min(allInteractions$Interaction),
...
for(indispecies in studySpecies){
  ...
  plot_dists <- dist(cbind(allInteractionswithLocation$Longitude,
                           allInteractionswithLocation$Latitude))
  Interaction_dists <- dist(allInteractionswithLocation$Interaction)
  a <- suppressWarnings(mantel.rtest(plot_dists, Interaction_dists, nrepet = 9999)$pvalue)
  mantelTestResults[[indispecies]] <- paste("Mantel~test:~italic(P)==", round(a, 2), sep = "")
  indispeciesLocations <- SpatialPoints(allInteractionswithLocation[,.(Easting, Northing)],
  ...
  indispeciesLocations$Interaction <- allInteractionswithLocation$Interaction
  ...
  allfigures[[indispecies]] <- ggplot(data = studyAreaall, aes(x = long, y = lat)) +
    ...
    scale_color_gradient2(low="red", mid = "gray", high = "green",
                          limits = c(min(allInteractions$Interaction),
                                     max(allInteractions$Interaction)),
                          breaks = seq(min(allInteractions$Interaction),
                                       max(allInteractions$Interaction),
                                       length = 5),
                          guide = "none")+
...
ggsave(file = file.path(workPath, "TablesFigures", 
                        paste("Figure S11. Spatial Autocorrelation check_Interaction.png")),
