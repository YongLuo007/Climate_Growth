...
for(indispecies in studySpecies){
  ...
  mainTrends$PredictedABGR <- treeToPlotConvertor*(exp(fittedvalues$fit)-15)
  mainTrends$PredictedABGR_Upper <- treeToPlotConvertor*(exp(fittedvalues$fit+1.98*fittedvalues$se.fit)-15)
  mainTrends$PredictedABGR_Lower <- treeToPlotConvertor*(exp(fittedvalues$fit-1.98*fittedvalues$se.fit)-15)
  ...
  if(nrow(speciesallHcoeff[Direction == "changewithH",]) == 1){
    ...
    changewithH$PredictedABGR <- exp(fittedvalues$fit)-15
    changewithH$PredictedABGR_Upper <- exp(fittedvalues$fit+1.98*fittedvalues$se.fit)-15
    changewithH$PredictedABGR_Lower <- exp(fittedvalues$fit-1.98*fittedvalues$se.fit)-15
...
figure <- ggplot(data = output[Direction != "mainTrend"], aes(x = Year, y = PredictedABGR))+
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.position = c(0.9, 0.30),
        legend.text = element_text(size = 13),
        legend.title = element_text(size = 15))
workPath <- "~/GitHub/Climate_Growth"
ggsave(file = file.path(workPath, "TablesFigures", 
                        paste("Figure S6. temporal trends with H including negative growth.png")),
