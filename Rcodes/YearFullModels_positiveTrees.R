...
analysesDataAll[,minABGR:=min(BiomassGR), by = uniTreeID]
analysesDataAll <- analysesDataAll[minABGR>0,]
...
for(indispecies in studySpecies){
  ...
  speciesDataAll[,':='(logY = log(BiomassGR), 
...
save.image(file.path(workPath, "data", selectionMethod,
                     "FullYearModels_positivetrees.RData"))
